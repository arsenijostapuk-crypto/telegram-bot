...
def handle_back(message):
    ...
    elif text == "Так, зрозуміло ✅":
        bot.send_message(chat_id, "Чудово! 😊", reply_markup=main_menu())
    else:  # "Назад ◀️"
...
def handle_all_messages(message):
    ...
    # Перевіряємо, чи користувач в режимі очікування замовлення
    if user_id in user_states and user_states[user_id] == "waiting_for_order":
        # Якщо натиснули "Скасувати замовлення ❌"
        if text == "Скасувати замовлення ❌":
            del user_states[user_id]
            bot.send_message(chat_id, "✅ Замовлення скасовано.", reply_markup=main_menu())
            return
        
        # Якщо це текст замовлення
        process_order(message)
        return
    
    # Якщо це не кнопка з меню - показуємо головне меню
